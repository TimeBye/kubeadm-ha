...
- 下载所需离线包
  ...
  # 集群运行所需的镜像
  curl -LO https://oss.choerodon.com.cn/kubeadm-ha/kubernetes-1.27.4-images-amd64.tgz
  # CentOS 7 系统对应软件包
  curl -LO https://oss.choerodon.com.cn/kubeadm-ha/centos7/kubernetes-1.27.4-centos7-amd64.tgz
  # 克隆脚本
...
- 准备 kubeadm-ha(ansible) 环境
  ```shell
  # 解压
  tar -xzvf kubernetes-1.27.4-images-amd64.tgz
  tar -xzvf kubernetes-1.27.4-centos7-amd64.tgz
...
- 编辑变量文件 `./my-cluster/variables.yaml`，内容如下
  ...
  # 注意地址末尾 / 必须加上
  base_yum_repo: http://yum.custom.local:12480/packages/
  epel_yum_repo: http://yum.custom.local:12480/packages/
  docker_yum_repo: http://yum.custom.local:12480/packages/
  kubernetes_yum_repo: http://yum.custom.local:12480/packages/
  # 若使用 Docker 作为运行时，则添加此配置
  cri_dockerd_download_url: http://yum.custom.local:12480/packages/cri-dockerd-0.3.4.amd64.tgz
  # 设置信任镜像库
...
- 安装 helm（可选项，可跳过）
  ...
  **注意：** 以下操作未特殊说明都在 `192.168.56.11` 第一台 master 节点执行。
  ...
  1. 下载helm客户端
      ...
      ```bash
      curl -sO http://yum.custom.local:12480/packages/helm-v3.12.3-linux-amd64.tar.gz
      ```
