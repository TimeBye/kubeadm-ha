...
- 编辑变量文件 `./my-cluster/variables.yaml`，内容如下
  ```yaml
  # 适配 Debian 系安装容器引擎后即启动，导致配置文件未生效
  ...
  cri_dockerd_download_url: http://pkgs.custom.local:12480/packages/cri-dockerd-0.3.4.amd64.tgz
  # 设置信任镜像库
...
- 安装 helm（可选项，可跳过）
  ...
  **注意：** 以下操作未特殊说明都在 `192.168.56.11` 第一台 master 节点执行。
  ...
  1. 下载helm客户端
      ...
      ```bash
      curl -sO http://pkgs.custom.local:12480/packages/helm-v3.12.3-linux-amd64.tar.gz
      ```
