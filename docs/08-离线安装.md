...
```shell
curl -L -o /kubernetes/helm-v3.2.4-linux-amd64.tar.gz \
    https://file.choerodon.com.cn/kubernetes-helm/v3.2.4/helm-v3.2.4-linux-amd64.tar.gz
```
...
> 也可点击这里获取[镜像离线包](https://github.com/TimeBye/kubeadm-ha/actions?query=workflow%3A%22Download+images%22)

#### 缓存文件列表

- 至此打包了以下缓存文件，将以下 4 个文件上传至搭建 ansible 环境的服务器上：
  - /kubernetes/kubeadm-ha.tar.gz
  - /kubernetes/kubernetes-1.20.1.tar
  - /kubernetes/kubernetes-1.20.1.tar.gz
  - /kubernetes/helm-v3.2.4-linux-amd64.tar.gz
...
- 准备 ansible 环境
  ```shell
  # 解压 kubernetes-1.20.1.tar.gz
  mkdir yum
  tar -xzvf kubernetes-1.20.1.tar.gz -C yum
  # 解压 kubernetes-1.20.1.tar.gz
  ...
  systemctl start docker
  # 加载 docker 镜像
  docker load -i kubernetes-1.20.1.tar
  ```
...
- 运行 ansible 镜像
  ...
  docker run -d --name ansible -p 12480:80 \
    ...
    -v $PWD/kubernetes-1.20.1.tar:/kubernetes/kubernetes-1.20.1.tar \
    -v $PWD/helm-v3.2.4-linux-amd64.tar.gz:/kubernetes/helm-v3.2.4-linux-amd64.tar.gz \
...
1. 下载helm客户端
    ...
    ```bash
    curl -L -o helm-v3.2.4-linux-amd64.tar.gz http://192.168.56.15:12480/helm-v3.2.4-linux-amd64.tar.gz
    ```
