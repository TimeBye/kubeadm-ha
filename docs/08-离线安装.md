...
- 下载所需离线包
  ```
  curl -kLO https://oss.choerodon.com.cn/kubeadm-ha/kubeadm-ha-1.21.4-amd64.tar
  curl -kLO https://oss.choerodon.com.cn/kubeadm-ha/docker-ce-20.10.7-amd64.tar.gz
  ```
...
- 准备 kubeadm-ha(ansible) 环境
  ...
  # 加载 kubeadm-ha 镜像
  docker load -i kubeadm-ha-1.21.4-amd64.tar
  ```
...
- 运行 kubeadm-ha 镜像
  ...
  docker run -d --restart=always --name kubeadm-ha \
    ...
    setzero/kubeadm-ha:1.21.4-amd64
  ```
