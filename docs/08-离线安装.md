...
- 下载所需离线包
  ```
  wget https://oss.choerodon.com.cn/kubeadm-ha/kubeadm-ha-1.20.2-amd64.tar
  wget https://oss.choerodon.com.cn/kubeadm-ha/docker-ce-20.10.2-amd64.tar.gz
  ```
...
- 准备 kubeadm-ha(ansible) 环境
  ```shell
  # 解压 docker-ce-20.10.2-amd64.tar.gz
  mkdir docker-ce
  tar -xzvf docker-ce-20.10.2-amd64.tar.gz -C docker-ce
  ...
  # 加载 kubeadm-ha 镜像
  docker load -i kubeadm-ha-1.20.2-amd64.tar
  ```
...
- 运行 kubeadm-ha 镜像
  ...
  docker run -d --restart=always --name kubeadm-ha \
    ...
    setzero/kubeadm-ha:1.20.2-amd64
  ```
