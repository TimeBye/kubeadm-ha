...
> 也可点击这里获取[镜像离线包](https://github.com/TimeBye/kubeadm-ha/actions?query=workflow%3ADownload)

#### 缓存文件列表

- 至此打包了以下缓存文件，将以下 4 个文件上传至搭建 ansible 环境的服务器上：
  - /kubernetes/kubeadm-ha.tar.gz
  - /kubernetes/kubernetes-1.19.4.tar
  - /kubernetes/kubernetes-1.19.4.tar.gz
...
- 准备 ansible 环境
  ```shell
  # 解压 kubernetes-1.19.4.tar.gz
  mkdir yum
  tar -xzvf kubernetes-1.19.4.tar.gz -C yum
  # 解压 kubernetes-1.19.4.tar.gz
  ...
  systemctl start docker
  # 加载 docker 镜像
  docker load -i kubernetes-1.19.4.tar
  ```
...
- 运行 ansible 镜像
  ...
  docker run -d --name ansible -p 12480:80 \
    ...
    -v $PWD/kubernetes-1.19.4.tar:/kubernetes/kubernetes-1.19.4.tar \
