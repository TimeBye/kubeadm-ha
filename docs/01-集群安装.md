...
- MacOS
  ```
  brew install https://raw.githubusercontent.com/Homebrew/homebrew-core/1bcdda7697f7293bc472dad662532e2d308cb110/Formula/ansible.rb
...
- 搭建集群后有以下两种“样式”显示，请自行选择：
  - 样式一
    ...
    192.168.56.11   Ready    etcd,master,worker   7m25s   v1.18.5
    192.168.56.12   Ready    etcd,master,worker   5m18s   v1.18.5
    192.168.56.13   Ready    etcd,master,worker   5m18s   v1.18.5
    192.168.56.14   Ready    worker               4m37s   v1.18.5
    ```

  - 样式二
    ```
    NAME    STATUS   ROLES                AGE     VERSION
    node1   Ready    etcd,master,worker   7m25s   v1.18.5
    node2   Ready    etcd,master,worker   5m18s   v1.18.5
    node3   Ready    etcd,master,worker   5m18s   v1.18.5
    node4   Ready    worker               4m37s   v1.18.5
    ```
